...
function LoginPage({ setAuthenticated }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const authentication = await authenticate(username);
      if (authentication) {
    ...
    <div className="h-screen w-screen bg-slate-700 justify-center flex items-center">
      <div className="wrapper w-80 bg-slate-800 p-10 rounded-md ">
        <form action="" onSubmit={handleSubmit}>
          ...
          <div className="input-box relative w-full h-14 mt-7">
            <span>
              <FaUserCircle className="icon self-center text-white absolute left-2 top-2.5" />
              <input
                type="text"
                placeholder="Amazon ID"
                className="w-full border-2 border-solid rounded-full px-7 py-1  bg-transparent text-slate-200"
                value={username}
          ...
          <div>
            <button
              type="submit"
              className="w-full h-8 bg-orange-400 hover:bg-orange-500 rounded-full shadow font-bold"
            >
              Login
            </button>
          </div>
