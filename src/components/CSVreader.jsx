...
function CSVreader() {
        ...
        if (file) {
            fileReader.onload = async function (event) {
                ...
                try {
                    ...
                    console.error('Error converting CSV to JSON or sending POST request:', error);
                }
            };
    ...
    return (
        <div style={{ textAlign: "center" }}>
            <h1>CSV IMPORT</h1>
            <form>
                <input type={"file"} id="filePicker" accept={".csv"} onChange={handleOnChange} />
                <button className='border border-black' onClick={(e) => handleOnSubmit(e)}>Submit !</button>
            </form>
        </div>
    );
}
